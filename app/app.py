...
@st.cache_data
def load_data():
    table    = pd.read_csv(os.path.join(DATA, 'kitui_wasi_ward_table.csv'))
    wasi_geo = json.load(open(os.path.join(DATA, 'kitui_wasi_ward.geojson')))
    hs_geo   = json.load(open(os.path.join(DATA, 'kitui_hotspot_ward.geojson')))
    for f in wasi_geo['features']:
        ward = f['properties']['Ward']
        row  = table[table['Ward'] == ward]
        if not row.empty:
            f['properties'].update({
                'C2_Rainfall':   float(row['C2_Rainfall'].iloc[0]),
                'C3_NDVI':       float(row['C3_NDVI'].iloc[0]),
                'C4_Population': float(row['C4_Population'].iloc[0]),
                'C5_Slope':      float(row['C5_Slope'].iloc[0]),
            })
...
filtered_wards = set(filtered['Ward'].tolist())
filtered_geo = {
    'type': 'FeatureCollection',
    'features': [f for f in wasi_geo['features']
...
with tab1:
    ...
    with col1:
        ...
        fg = json.loads(json.dumps(filtered_geo))
    ...
    with col2:
        ...
        cc = table['Stress_Class'].value_counts().reindex(
            ['Very High', 'High', 'Moderate', 'Low', 'Very Low'], fill_value=0
        ).reset_index()
        cc.columns = ['Class', 'Count']
        fig_bar = px.bar(cc, x='Count', y='Class', orientation='h',
                         color='Class', color_discrete_map=STRESS_COLOURS, text='Count')
...
with tab2:
    ...
    labels     = ['C2: Rainfall (35.7%)', 'C3: NDVI (21.4%)',
    ...
    for col, w, colour, label in zip(comp_cols, weights, colours, labels):
        ...
        fig_comp.add_trace(go.Bar(y=comp_sorted['Ward'], x=vals, name=label,
                                  orientation='h', marker_color=colour, marker_opacity=0.85))
    fig_comp.add_trace(go.Scatter(
        y=comp_sorted['Ward'], x=comp_sorted['WASI_mean'],
        mode='markers', name='WASI composite',
        marker=dict(color='black', size=5, symbol='circle'),
    ))
    fig_comp.update_layout(
        barmode='stack', height=900,
        margin=dict(l=0, r=0, t=10, b=0),
        xaxis_title='Weighted stress contribution', yaxis_title='',
    ...
    selected_ward = st.selectbox('Ward', options=sorted(table['Ward'].tolist()))
    ...
    fig_radar = go.Figure(go.Scatterpolar(
        r=[ward_row['C2_Rainfall'], ward_row['C3_NDVI'],
           ward_row['C4_Population'], ward_row['C5_Slope'], ward_row['C2_Rainfall']],
        theta=['C2 Rainfall', 'C3 NDVI', 'C4 Population', 'C5 Slope', 'C2 Rainfall'],
        fill='toself', fillcolor='rgba(46, 117, 182, 0.3)',
        line=dict(color='#2E75B6'), name=selected_ward
    ))
...
with tab3:
    ...
    with hc2:
        ...
        hc = hs_df['Ward_Class'].value_counts().reset_index()
        hc.columns = ['Class', 'Wards']
        fig_hb = px.bar(hc, x='Wards', y='Class', orientation='h',
                        color='Class', color_discrete_map=HOTSPOT_COLOURS, text='Wards')
        fig_hb.update_layout(showlegend=False, height=200,
                             margin=dict(l=0, r=0, t=10, b=0),
                             xaxis_title='Number of wards', yaxis_title='')
...
| Coldspot 95% (z < -1.960) | 21.2% |
| Coldspot 99% (z < -2.576) | 16.2% |
        """)
        st.subheader('Hotspot ward detail')
        hd = hs_df[hs_df['Ward_Class'] == 'Hotspot'][
            ['Ward', 'WASI_mean', 'Gi_Mean', 'Hotspot_Pct']].copy()
        hd.columns = ['Ward', 'WASI', 'Gi* Mean', 'Hotspot %']
        st.dataframe(hd.round(3), use_container_width=True, hide_index=True)
...
with tab4:
    ...
    st.dataframe(
        display, use_container_width=True, height=600,
        column_config={
    ...
    tc1.download_button('Download ward table (CSV)', data=table.to_csv(index=False),
                        file_name='kitui_wasi_ward_table.csv', mime='text/csv')
